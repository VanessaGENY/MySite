...
const Game = () => {
    ...
    const addNextButton = () => {
        return (<button className="gamelist-button next-game-button" onClick={getNextGame}>&#5171;</button>)
    }

    const addPreviousButton = () => {
        return (<button className="gamelist-button previous-game-button" onClick={getPreviousGame}>&#5176;</button>)
    }
    ...
    const isDisplayed = (index) => {
        return index === currentIndex || index === getNextIndex() || index === getPreviousIndex();
    }
    ...
    return (
        <div className="games" style={{ backgroundImage: `url("${img.src}")`, transition: "all .5s ease", }}>
            ...
            <div className={`hidden-game previousprevious-game ${currentAnimation}-previousprevious`}>
